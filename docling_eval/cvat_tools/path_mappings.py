...
def validate_merge_paths(
    ...
    Rules:
    1. Elements connected by a merge path must have same label and content_layer
    2. A merge path must never skip elements that are connected by a reading-order path
    ...
    for path_id, el_ids in merge_mappings.items():
        if len(el_ids) < 2:
            continue
        ...
        for el_id in el_ids[1:]:
            ...
            if el.label != first_el.label:
                errors.append(
                    f"Merge path {path_id}: Elements have different labels: {first_el.label} vs {el.label}"
                )
            if el.content_layer != first_el.content_layer:
                errors.append(
                    f"Merge path {path_id}: Elements have different content layers: {first_el.content_layer} vs {el.content_layer}"
                )
...
def validate_group_paths(
    ...
    Rules:
    1. Elements connected by a group path must have same label and content_layer
    ...
    for path_id, el_ids in group_mappings.items():
        if len(el_ids) < 2:
            continue
        ...
        for el_id in el_ids[1:]:
            ...
            if el.label != first_el.label:
                errors.append(
                    f"Group path {path_id}: Elements have different labels: {first_el.label} vs {el.label}"
                )
            if el.content_layer != first_el.content_layer:
                errors.append(
                    f"Group path {path_id}: Elements have different content layers: {first_el.content_layer} vs {el.content_layer}"
                )
